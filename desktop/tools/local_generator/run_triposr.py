...
def run_triposr_single(image_path: str, out_glb: str, preset: str, requested_device: str) -> None:
...
def score_glb_candidate(out_glb: str) -> Tuple[float, str]:
    try:
        import trimesh  # type: ignore
    except Exception:
        size_kb = float(os.path.getsize(out_glb)) / 1024.0 if os.path.exists(out_glb) else 0.0
        return size_kb, f"score_fallback=size_kb:{size_kb:.1f}"

    loaded = trimesh.load(out_glb, file_type="glb", force="scene")
    if isinstance(loaded, trimesh.Scene):
        geometries = [g for g in loaded.geometry.values() if isinstance(g, trimesh.Trimesh)]
        if not geometries:
            return -999.0, "empty_scene"
        mesh = trimesh.util.concatenate(geometries)
    elif isinstance(loaded, trimesh.Trimesh):
        mesh = loaded
    else:
        return -999.0, "invalid_geometry"

    face_count = int(len(mesh.faces))
    vertex_count = int(len(mesh.vertices))
    extents = list(getattr(mesh, "extents", [1.0, 1.0, 1.0]))
    valid_extents = [max(float(value), 1e-6) for value in extents]
    max_extent = max(valid_extents)
    min_extent = min(valid_extents)
    bbox_ratio = max_extent / max(min_extent, 1e-6)
    watertight_bonus = 0.25 if bool(getattr(mesh, "is_watertight", False)) else 0.0
    size_kb = float(os.path.getsize(out_glb)) / 1024.0 if os.path.exists(out_glb) else 0.0

    face_score = min(float(face_count) / 220000.0, 1.0)
    ratio_score = max(0.0, 1.0 - min(max(bbox_ratio - 10.0, 0.0) / 18.0, 1.0))
    size_score = min(size_kb / 900.0, 1.0)
    total_score = face_score * 0.52 + ratio_score * 0.30 + size_score * 0.18 + watertight_bonus
    summary = (
        f"score={total_score:.4f} faces={face_count} verts={vertex_count} "
        f"bbox_ratio={bbox_ratio:.3f} watertight={bool(getattr(mesh, 'is_watertight', False))} size_kb={size_kb:.1f}"
    )
    return total_score, summary


def collect_multiview_images(views_dir: str) -> List[str]:
    expected_suffixes = ["front", "front_left", "left", "back", "right", "front_right"]
    ordered_paths: List[str] = []

    for index, suffix in enumerate(expected_suffixes):
        candidate = os.path.join(views_dir, f"view_{index:02d}_{suffix}.png")
        if os.path.exists(candidate):
            ordered_paths.append(candidate)

    if len(ordered_paths) >= 3:
        return ordered_paths[:6]
    ...
    for name in sorted(os.listdir(views_dir)):
        ...
        if lower.endswith(".png") or lower.endswith(".jpg") or lower.endswith(".jpeg") or lower.endswith(".webp"):
            all_images.append(os.path.join(views_dir, name))

    deduped: List[str] = []
    seen = set()
    for item in ordered_paths + all_images:
    ...
    if len(deduped) < 3:
        raise RuntimeError(f"Multiview requires at least 3 images in {views_dir}. Found {len(deduped)}")
    return deduped[:6]
...
def run_triposr(
    ...
    views_dir: str = "",
    combine_mode: str = "best_of_views",
) -> None:
    if combine_mode != "best_of_views":
        raise RuntimeError(f"Unsupported combine_mode: {combine_mode}")

    has_multiview = bool(views_dir.strip()) and os.path.isdir(views_dir)
    if not has_multiview:
        run_triposr_single(image_path, out_glb, preset, requested_device)
        return

    view_paths = collect_multiview_images(views_dir)
    emit("multiview", 40, f"Using {len(view_paths)} multiview inputs (best-of-views)")
    print(f"[TRIPOSR][MULTIVIEW] mode={combine_mode} views={len(view_paths)} dir={views_dir}", flush=True)

    global SUPPRESS_PROGRESS
    candidate_records: List[Tuple[float, str, str, str]] = []
    errors: List[str] = []

    for index, view_path in enumerate(view_paths):
        stage_percent = 42 + int((index / max(1, len(view_paths))) * 44)
        emit("multiview", stage_percent, f"TripoSR candidate {index + 1}/{len(view_paths)}: {os.path.basename(view_path)}")
        candidate_out = f"{out_glb}.mv{index:02d}.glb"
        previous_suppression = SUPPRESS_PROGRESS
        SUPPRESS_PROGRESS = True
        try:
            run_triposr_single(view_path, candidate_out, preset, requested_device)
            score, summary = score_glb_candidate(candidate_out)
            print(
                f"[TRIPOSR][MULTIVIEW] candidate={os.path.basename(view_path)} out={candidate_out} {summary}",
                flush=True,
            )
            candidate_records.append((score, candidate_out, view_path, summary))
        except Exception as error:
            error_message = f"{os.path.basename(view_path)}: {error}"
            print(f"[TRIPOSR][MULTIVIEW][ERR] {error_message}", file=sys.stderr, flush=True)
            errors.append(error_message)
        finally:
            SUPPRESS_PROGRESS = previous_suppression

    if not candidate_records:
        details = errors[0] if errors else "unknown failure"
        raise RuntimeError(f"Multiview TripoSR failed for all candidates. {details}")

    candidate_records.sort(key=lambda item: item[0], reverse=True)
    best_score, best_glb, best_view, best_summary = candidate_records[0]
    shutil.copyfile(best_glb, out_glb)
    emit("multiview", 92, f"Selected best view: {os.path.basename(best_view)} (score={best_score:.4f})")
    print(f"[TRIPOSR][MULTIVIEW] selected={best_view} summary={best_summary}", flush=True)

    for _, candidate_glb, _, _ in candidate_records:
        try:
            os.remove(candidate_glb)
        except Exception:
            pass
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--views_dir", default="")
    parser.add_argument("--combine_mode", choices=["best_of_views"], default="best_of_views")
    return parser
...
def main() -> int:
    ...
    try:
        run_triposr(args.image, out_glb, args.preset, args.device, args.views_dir, args.combine_mode)
        return 0
    except Exception as error:
