...
function navItemClass(isActive: boolean) {
  return `group relative flex items-center gap-3 rounded-xl border px-3 py-2 text-sm transition-colors duration-150 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] ${
    isActive
      ? "border-[var(--border)] bg-[var(--surface-2)] text-[var(--text)] before:absolute before:inset-y-2 before:left-0 before:w-px before:rounded-full before:bg-[var(--accent)]"
      ...
      <nav className="space-y-1.5">
        ...
        >
          <span className={iconClass}>
            <BrandLogo size={12} showText={false} label="VOLUMIA" />
          </span>
          {t("nav.dashboard")}
        </NavLink>
        <NavLink
